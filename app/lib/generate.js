...
const ultronPath = path.dirname(__dirname)
...
export default {
  ...
  transport (data, filename, groupPath) {
    ...
    var $ = cheerio.load(data.replace(/'<'/g, '\'&lt\''), {decodeEntities: false})
    ...
    if (isArticleList) {
      // include 文章列表页
      var articleFragment1 = jetpack.read(ultronPath + '/static/template/inc-article-list/inc-article-list-1.shtml')
      // ifCond传递'<' 引起dom节点匹配问题，暂时用转换字符解决
      if (articleFragment1) {
        $('div.pagination').length && $('div.pagination').first().after('\r\n' + articleFragment1.replace('\'<\'', '\'&lt\'') + '\r\n').remove()
    ...
    $('[cms-data-type]').each(function (i, el) {
      let type = $(this).attr('cms-data-type').replace(/^list-\w+$/, 'list-content')
      let cid = $(this).attr('cms-data-component-cid')
      let sameid = $(this).attr('cms-data-component-sameid')
      let nodeType = $(this).attr('cms-node-type')
      ...
      if (type !== 'html') {
        ...
        let content = $(plum).attr('cms-data-type', type).append($(this).clone())
        if (nodeType) {
        ...
        var src = join(ultronPath, 'static', 'template', 'component', type)
        let index = typeof cid !== 'undefined' ? cid : componentIds[type]++
        let componentValue = `${type}-${index}`
        console.log('groupPath = ', groupPath, 'and componentValue = ', componentValue)
        var dst = join(groupPath, componentValue)
        console.log('component dist ', dst)
        ...
        ncp(src, dst, (err) => {
          ...
          jetpack.readAsync(join(src, 'component.handlebars'))
            .then(contents => {
              var htm = content.html().split(/\r\n/)
              var re = /^(\s+)/
              var spaces = 0
              htm[htm.length - 1] = htm[htm.length - 1].replace(re, (match, $1) => {
                spaces = $1.length
                return ''
              })
              ...
              jetpack.write(join(dst, 'component.handlebars'), contents + '\r\n' + htm.join('\r\n'))
            })
        })
      } else {
        let content = $(plum).attr('cms-data-type', type).append($(this).clone())
